...
getgenv().Silent = {
    Settings = {
        Toggled = false, -- Toggled default to false
        ...
        Circle = {
            Visible = false, -- Circle visibility default to false
...
CreateMain("Silent Aim Settings") -- Main window for Silent Aim settings

local tab1 = CreateTab("General Settings") -- Tab for general settings

CreateToggle(tab1, "Silent Aim Toggle", "Enable/Disable Silent Aim", function(toggled)
    getgenv().Silent.Settings.Toggled = toggled
end):Set(getgenv().Silent.Settings.Toggled)

local aimPartOptions = {"HumanoidRootPart", "UpperTorso", "LowerTorso", "Head"}
CreateSideDropButton(tab1, "Aim Part", aimPartOptions, function(selectedOption)
    getgenv().Silent.Settings.AimPart = selectedOption
end):Set(getgenv().Silent.Settings.AimPart)

local predictionOptions = {"Enable", "Disable"}
CreateSideDropToggle(tab1, "Prediction", predictionOptions, function(selectedOption)
    getgenv().Silent.Settings.Prediction.Toggled = (selectedOption == "Enable")
end):Set(getgenv().Silent.Settings.Prediction.Toggled)

CreateMain("FOV Circle Settings") -- Main window for FOV circle settings

local tab2 = CreateTab("FOV Circle Settings") -- Tab for FOV circle settings

CreateToggle(tab2, "Circle Visibility", "Enable/Disable FOV Circle", function(toggled)
    getgenv().Silent.Settings.Circle.Visible = toggled
    FOVCircle.Visible = toggled
end):Set(getgenv().Silent.Settings.Circle.Visible)

local colorTextBox = CreateTextBox(tab2, "Circle Color (RGB)", "255, 255, 255", function(text)
    local r, g, b = text:match("(%d+),(%d+),(%d+)")
    if r and g and b then
        getgenv().Silent.Settings.Circle.Color = Color3.fromRGB(tonumber(r), tonumber(g), tonumber(b))
        FOVCircle.Color = getgenv().Silent.Settings.Circle.Color
    end
end)

local transparencyTextBox = CreateTextBox(tab2, "Circle Transparency", tostring(getgenv().Silent.Settings.Circle.Transparency), function(text)
    local transparency = tonumber(text)
    if transparency then
        getgenv().Silent.Settings.Circle.Transparency = transparency
        FOVCircle.Transparency = transparency
    end
end)

local thicknessTextBox = CreateTextBox(tab2, "Circle Thickness", tostring(getgenv().Silent.Settings.Circle.Thickness), function(text)
    local thickness = tonumber(text)
    if thickness then
        getgenv().Silent.Settings.Circle.Thickness = thickness
        FOVCircle.Thickness = thickness
    end
end)

local radiusTextBox = CreateTextBox(tab2, "Circle Radius", tostring(getgenv().Silent.Settings.Circle.Radius), function(text)
    local radius = tonumber(text)
    if radius then
        getgenv().Silent.Settings.Circle.Radius = radius
        FOVCircle.Radius = playerCamera.ViewportSize.X / radius
    end
end)

local filledToggle = CreateToggle(tab2, "Circle Filled", "Toggle filled/unfilled circle", function(toggled)
    getgenv().Silent.Settings.Circle.Filled = toggled
    FOVCircle.Filled = toggled
end):Set(getgenv().Silent.Settings.Circle.Filled)

-- Function to update FOV circle position every frame
